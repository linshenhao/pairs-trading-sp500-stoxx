...
### 为什么增加 Sharpe 矩阵和成本曲线？

更新后的 [S&P 500 notebook](notebooks/SP500_Improved.ipynb) 和 [STOXX 600 notebook](notebooks/STOXX.ipynb) 增加了 **3 × 3 费后 Sharpe 矩阵**。`Entry` 决定两只股票偏离多远才开仓；`Exit` 决定价格关系恢复到什么程度才平仓。把两者组合起来观察，可以判断结果是否在相邻参数下也较稳定，还是只靠某一个参数组合才好看。测试时保持已选出的配对不变，因此这里检验的是交易规则，而不是重新选股票。
...
- **左图 — Sharpe 矩阵：** 每一行是开仓阈值（Entry），每一列是平仓阈值（Exit）。每个格子代表该组合在扣除交易成本后的年化 **净 Sharpe ratio**，数值越高越好。绿色深浅只用于同一张市场图内部的视觉提示；比较 S&P 500 和 STOXX 时应看格子中的数值，而不是直接比较颜色。
- **右图 — 成本曲线：** 横轴是每单位换手的估计成本（bps），纵轴是净 Sharpe。当曲线跌破零，表示合理范围内的交易成本已经足以消除该策略的历史风险调整后收益。
- **为什么做这个：** 这两张图用于检查结果是否能承受相邻交易规则和更现实的执行成本。如果策略只在一个孤立格子内有效，或只在不现实的低成本下有效，那么它的表现很脆弱，实盘中很可能无法维持。
